...
int main() {
  ...
  __builtin_enable_interrupts();  // INT step 7: enable interrupts at CPU
    ...
    //LCD_drawChar(0,0, 'G', BLACK, WHITE);
    char msg[50];
    char fps[50];
    float elapsed = 0.0;
    while(1) {
        ...
        for (ii = 0; ii < 100; ii++){
            _CP0_SET_COUNT(0);
            sprintf(msg,"Hello World %d!",ii);
            LCD_drawString(28,32, msg, BLACK, WHITE);

            LCD_drawBar(14,40,ii,BLACK,CYAN);
            sprintf(fps,"FPS: %f!",elapsed);
            LCD_drawString(20,70, fps, BLACK, WHITE);
            elapsed = 1.0/(_CP0_GET_COUNT() / 24000000.0);
